...
public abstract class AbstractIam implements Provider, Identity, AutoCloseable {
    ...
    public void attachInlinePolicy(AttachInlinePolicyRequest request) {
        ...
        if (request.getPolicyDocument() == null || request.getPolicyDocument().getStatements() == null || request.getPolicyDocument().getStatements().isEmpty()) {
            throw new InvalidArgumentException("policy document and/or policy statements cannot be null");
        }
